...
public class ReadReqRecharge {
	private static final Logger LOGGER = LoggerFactory.getLogger(GeneratorTest.class);
	public static void main(String[] args) {
		 try
	      {
	      PrintWriter pw = new PrintWriter(new FileOutputStream("D:\\ReqRecharge.txt"));
	      pw.close();
	      }
	      catch(Exception e)
	      {
	      }
		 String fileName = "D:/ReqRechargePr.txt";  
		 readNameType(fileName);
		 readFileByLines(fileName);
		System.out.println("}");
		 try
         {
         FileWriter fw = new FileWriter(new File("D:\\ReqRecharge.txt"),true);
	             ...
	             while ((tempString = reader.readLine()) != null) {  
	            	 ...
	            	 if(tempString.indexOf("|")!=-1&!tempString.contains("serialVersionUID")){
		            	 ...
		            	 String type = tempString.toString().substring(tempString.indexOf("|")+2);
		                   ...
		                   PrintWriter pw = new PrintWriter(fw);
		                   //字符串处理得到set get方法
		                   pw.println("public " +type +" get"+aaa.substring(0,1).toUpperCase().concat(aaa.substring(1).toLowerCase())+"()"+"\n"+"{"+"\nreturn this."+aaa+";"+"\n}");
		            		 pw.println("public void" +" set"+aaa.substring(0,1).toUpperCase().concat(aaa.substring(1).toLowerCase())+"("+type+" "+aaa+")"+"\n"+"{"+"\n this."+aaa+" = "+aaa+";"+"\n}");
		                   pw.close();
		                   }
		                   catch(Exception e)
		                   {
	            		 ...
	            		 System.out.println("public " +type +" get"+aaa.substring(0,1).toUpperCase().concat(aaa.substring(1).toLowerCase())+"()"+"\n"+"{"+"\nreturn this."+aaa+";"+"\n}");
	            		 System.out.println("public void" +" set"+aaa.substring(0,1).toUpperCase().concat(aaa.substring(1).toLowerCase())+"("+type+" "+aaa+")"+"\n"+"{"+"\n this."+aaa+" = "+aaa+";"+"\n}");
	            	 }
