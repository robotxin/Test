...
public class ReadRespQrSession {
	private static final Logger LOGGER = LoggerFactory.getLogger(GeneratorTest.class);
	public static void main(String[] args) {
		 try
	      {
	      PrintWriter pw = new PrintWriter(new FileOutputStream("D:\\RespQrSession.txt"));
	      pw.close();
	      }
	      catch(Exception e)
	      {
	      }
		 String fileName = "D:/RespQrSessionPr.txt";  
		 readNameType(fileName);
		 readFileByLines(fileName);
		System.out.println("}");
		 try
         {
         FileWriter fw = new FileWriter(new File("D:\\RespQrSession.txt"),true);
    ...
    public static void readNameType(String fileName) {  
        ...
        //	打印包名 类名
        try {  
       	 String classname = "RespQrSession";
         try
         {
         FileWriter fw = new FileWriter(new File("D:\\RespQrSession.txt"),true);
         PrintWriter pw = new PrintWriter(fw);
